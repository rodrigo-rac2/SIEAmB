...
- Prototype was shared with Najara and Thays (the clients) for feedback on 2026-08-30. **Awaiting their feedback and real content.**
...
1. **Collect real content from the committee** (dates, fees, areas, committee, email, about text) → replace fixtures
2. **Apply official visual identity** when design team delivers → tokens.css + logo
3. **Supabase project** (sa-east-1, free tier): persist registrations + news, admin login for the committee (see docs/IMPLEMENTATION_PLAN.md §4, tasks T0.7–T0.10)
4. **Phase 1**: Express API, admin panel, emails (Resend), Vercel cutover — plan §5
5. Later: submissions (§6), Mercado Pago payments (§7), certificates/anais (§8)
